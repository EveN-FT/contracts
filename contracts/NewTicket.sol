...
abstract contract ERC721Ticket is ERC721 {
    ...
    function _address(uint256 tokenId) internal view returns (address) {
        require(_exists(tokenId), "ERC721AddressStorage: address query for nonexistent token");
    ...
    function _setAddress(uint256 tokenId, address _addr) internal virtual {
        require(_exists(tokenId), "ERC721AddressStorage: address set of nonexistent token");
...
contract Ticket is ERC721Ticket{
    ...
    function mintGA(address eventAddr, string memory tokenURI, uint256 amount)
        public
